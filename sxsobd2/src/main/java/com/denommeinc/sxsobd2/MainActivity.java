...
import static com.denommeinc.sxsobd2.R.id;
import static com.denommeinc.sxsobd2.R.layout;
import static com.denommeinc.sxsobd2.R.string;
import static com.denommeinc.sxsobd2.R.style;
...
public class MainActivity extends PluginManager
    ...
    private static final int REQUEST_GRAPH_DISPLAY_LIST = 7;
    private static final int REQUEST_GRAPH_DISPLAY_HEADUP = 8;
    private static final int REQUEST_GRAPH_DISPLAY = 19;
    ...
    * */
    public static DeluxeSpeedView speedView;
    ...
    * Current ECU status
    * */
    public boolean isEcuConnected(ElmProt.STAT status) {
        int progress = 0;
    ...
    private transient final Handler mHandler = new Handler(Looper.getMainLooper())
    {
        ...
        public void handleMessage(Message msg)
        {
            try
            {
                ...
                log.log(Level.FINEST, String.format("Handler notification: %s", msg));

                switch (msg.what)
                {
                    case MESSAGE_STATE_CHANGE:
                        // log trace message for received handler notification event
                        log.log(Level.FINEST, String.format("State change: %s", msg));
                    ...
                    case MESSAGE_FILE_READ:
                        ...
                        setObdService(CommService.elm.getService(), getString(string.saved_data));
                    ...
                    case MESSAGE_DEVICE_NAME:
                        ...
                        Toast.makeText(getApplicationContext(),
                                getString(string.connected_to) + mConnectedDeviceName,
                    ...
                    case MESSAGE_OBD_STATE_CHANGED:
                        ...
                        * */
                        ...
                        {
                            setStatus(getResources().getStringArray(array.elmcomm_states)[state
                                    .ordinal()]);
                        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /*
        * Most views are handled by the dlgBuilder
        * */
        ...
        mPidAdapter = new ObdItemAdapter(this, layout.obd_item, ObdProt.PidPvs);
        mVidAdapter = new VidItemAdapter(this, layout.obd_item, ObdProt.VidPvs);
        mTidAdapter = new TidItemAdapter(this, layout.obd_item, ObdProt.VidPvs);
        mDfcAdapter = new DfcItemAdapter(this, layout.obd_item, ObdProt.tCodes);
        mPluginDataAdapter = new PluginDataAdapter(this, layout.obd_item, mPluginPvs);
        ...
        speedView = findViewById(id.speedView);
    ...
    public void onBackPressed()
    {
        ...
        } else
        {
            ...
            } else
            {
                ...
                {
                    exitToast =
                            Toast.makeText(this, string.back_again_to_exit, Toast.LENGTH_SHORT);
                    exitToast.show();
    ...
    public boolean onCreateOptionsMenu(Menu menu)
    {
        ...
        getMenuInflater().inflate(R.menu.obd_services, menu.findItem(id.obd_services).getSubMenu());
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.connect_fab) {
            ...
            handler.postDelayed(() -> {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    connect_fab.setBackgroundColor(getColor(color.green));
                }
            },2000);
    ...
    public boolean onOptionsItemSelected(MenuItem item)
    {
        ...
        {
            case id.day_night_mode:
                // toggle night mode setting
                Toast.makeText(this, "day_night_mode", Toast.LENGTH_SHORT).show();
                prefs.edit().putBoolean(NIGHT_MODE, !nightMode).apply();
                return true;

            case id.secure_connect_scan:
                Toast.makeText(this, "secure_connect_scan", Toast.LENGTH_SHORT).show();
                setMode(MODE.ONLINE);
                return true;

            case id.reset_preselections:
                Toast.makeText(this, "reset_preselections", Toast.LENGTH_SHORT).show();
                clearPreselections();
                recreate();
                return true;
            ...
            case id.plugin_manager:
                setManagerView();
                Toast.makeText(this, "plugin_manager", Toast.LENGTH_SHORT).show();
                return true;
            ...
            case id.service_none:
                setObdService(ObdProt.OBD_SVC_NONE, item.getTitle());
                Toast.makeText(this, "service_none", Toast.LENGTH_SHORT).show();
                return true;

            case id.service_data:
                setObdService(ObdProt.OBD_SVC_DATA, item.getTitle());
                Toast.makeText(this, "service_data", Toast.LENGTH_SHORT).show();
                return true;

            case id.service_vid_data:
                setObdService(ObdProt.OBD_SVC_VEH_INFO, item.getTitle());
                Toast.makeText(this, "service_vid_data", Toast.LENGTH_SHORT).show();
                return true;

            case id.service_freezeframes:
                setObdService(ObdProt.OBD_SVC_FREEZEFRAME, item.getTitle());
                return true;

            case id.service_testcontrol:
                setObdService(ObdProt.OBD_SVC_CTRL_MODE, item.getTitle());
                Toast.makeText(this, "service_test_control", Toast.LENGTH_SHORT).show();
                return true;

            case id.service_codes:
                setObdService(ObdProt.OBD_SVC_READ_CODES, item.getTitle());
                Toast.makeText(this, "service_codes", Toast.LENGTH_SHORT).show();
                return true;

            case id.service_clearcodes:
                clearObdFaultCodes();
                setObdService(ObdProt.OBD_SVC_READ_CODES, item.getTitle());
...
//                startActivityForResult(dashboardIntent, REQUEST_DASHBOARD_DISPLAY);
        ...
        switch (requestCode)
        {
            ...
            case REQUEST_SELECT_FILE:
                if (resultCode == RESULT_OK)
                {
                    ...
                    setMenuItemEnable(id.save, false);
                    setMenuItemEnable(id.obd_services, true);
                }
                break;
            ...
            case REQUEST_GRAPH_DISPLAY_LIST:
                // let context know that we are in list mode again ...
                dataViewMode = DATA_VIEW_MODE.LIST;
                break;

            case REQUEST_GRAPH_DISPLAY:
                dataViewMode = DATA_VIEW_MODE.CHART;
                break;

            case REQUEST_GRAPH_DISPLAY_HEADUP:
                // Display ChartActivity
                dataViewMode = DATA_VIEW_MODE.HEADUP;
                break;
            case REQUEST_DASHBOARD_DISPLAY:
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id)
    {
        ...
        switch (CommService.elm.getService())
        {
            ...
            case ObdProt.OBD_SVC_VEH_INFO:
                ...
                // Show Toast message
                Toast.makeText(this, string.copied_to_clipboard, Toast.LENGTH_SHORT).show();
                break;
    ...
    private void selectEcu(final Set<Integer> ecuAdresses)
    {
        ...
        if (ecuAdresses.size() > 1)
        {
            ...
            } else
            {
                ...
                // show dialog ...
                dlgBuilder
                        .setTitle(string.select_ecu_addr)
    ...
    protected void setNightMode(boolean nightMode)
    {
        ...
        setTheme(nightMode ? style.AppTheme_Dark : style.AppTheme);
    ...
    * */
    private void setNumCodes(int newNumCodes)
    {
        ...
        NEW_CODES = newNumCodes;
        READ_CODES = ObdProt.OBD_SVC_READ_CODES;
        PENDING_CODES = ObdProt.OBD_SVC_PENDINGCODES;
        PERMA_CODES = ObdProt.OBD_SVC_PERMACODES;
    ...
    private void setAutoHider(boolean active)
    {
        ...
        if (active)
        {
            long timeout = getPrefsInt(MainActivity.PREF_AUTOHIDE_DELAY, 15);
        ...
        if (mode != this.mode || mode == MODE.FILE)
        {
            ...
            switch (mode)
            {
                case OFFLINE:
                    ...
                    setMenuItemVisible(id.disconnect, false);
                    setMenuItemVisible(id.secure_connect_scan, true);
                    setMenuItemEnable(id.obd_services, false);
                    break;
                ...
                case ONLINE:
                    switch (CommService.medium)
                    {
                        case BLUETOOTH:
                            ...
                            {
                                Toast.makeText(this, getString(string.none_found), Toast.LENGTH_SHORT).show();
                                mode = MODE.OFFLINE;
                            } else
                            {
    ...
    /**
     * set measurement conversion system to metric/imperial
     *
     * @param cnvId ID for metric/imperial conversion
     */
    private void setConversionSystem(int cnvId)
    {
        log.info("Conversion: " + getResources().getStringArray(array.measure_options)[cnvId]);
    ...
    private void stopDemoService()
    {
        if (getMode() == MODE.DEMO)
        {
            ...
            Toast.makeText(this, getString(string.demo_stopped), Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void startDemoService()
    {
        if (getMode() != MODE.DEMO)
        {
            setStatus(getString(string.demo));
            Toast.makeText(this, getString(string.demo_started), Toast.LENGTH_SHORT).show();

            boolean allowConnect = mBluetoothAdapter != null && mBluetoothAdapter.isEnabled();
            setMenuItemVisible(id.secure_connect_scan, allowConnect);
            setMenuItemVisible(id.disconnect, !allowConnect);
    ...
    * */
    private void showStatus(CharSequence charSequence) {
        try {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    connect_fab.setText(charSequence);
                }
            });
    ...
    private void connectBtDevice(String address, boolean secure)
    {
        ...
        if (device.getAddress() != null) {
            Log.i("STATUS", "DEVICE CONNECTED" + "\t Secure Connection = " + secure);
        }
    }
...
//        }
        ...
        Spinner ff_selector = findViewById(id.ff_selector);
        ...
        switch (newObdService)
        {
            ...
            case ObdProt.OBD_SVC_READ_CODES:
                ...
                Toast.makeText(this, getString(string.long_press_dfc_hint), Toast.LENGTH_LONG).show();
                break;
    ...
    private void onConnect()
    {
        ...
        setMenuItemVisible(id.secure_connect_scan, false);
        setMenuItemVisible(id.disconnect, true);
    ...
    private void clearObdFaultCodes()
    {
        dlgBuilder
                .setIcon(android.R.drawable.ic_dialog_info)
                .setTitle(string.obd_clearcodes)
                .setMessage(string.obd_clear_info)
    ...
    private void confirmObdTestControl(String testControlName, int service, int tid)
    {
        dlgBuilder
                ...
                .setMessage(string.obd_test_confirm)
                .setPositiveButton(android.R.string.yes,
                        new DialogInterface.OnClickListener()
                        {
                            @Override
    ...
    private void runObdTestControl(String testControlName, int service, int tid)
    {
        ...
        dlgBuilder
                .setIcon(android.R.drawable.ic_dialog_info)
                .setTitle(testControlName)
                .setMessage(string.obd_test_progress)
                    ...
                    if (getListView().getCheckedItemCount() > 0)
                    {
                        ...
                        intent.putExtra(DashBoardActivity.RES_ID,
                                dataViewMode == DATA_VIEW_MODE.DASHBOARD
                                        ? layout.dashboard
                                        : layout.head_up);
                        startActivityForResult(intent, REQUEST_GRAPH_DISPLAY_LIST);
                        this.dataViewMode = dataViewMode;
                    }
                    break;
